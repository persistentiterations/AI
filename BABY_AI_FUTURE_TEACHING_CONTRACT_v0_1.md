...
repair (this `MARK_RESOLVE_DEPWALK` tranche) and any later tranche.
...
## Repair boundary for this tranche

- Repair ONLY the walk's contradiction reading (READ_CURRENT via plast).
- Do NOT open the two logged secondary divergences here (SUPERSEDE+RESOLVE
  surface-vs-oracle; repeated-MARK+one-RESOLVE). They are separate tranches
  with their own gates. (See the witness doc's "Not repaired here".)

## Verification contract

Any future session claiming completion of this tranche must reproduce:
